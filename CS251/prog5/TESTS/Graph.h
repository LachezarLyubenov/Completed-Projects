 ...
 */
/*
pathHelper(vector<vertex> vertices, std::vector<vertex_label> &report)
{
                ...
                for(edge &e : vertices[u].outgoing)
                {
                        ...
                        if(report[v].state == UNDISCOVERED)
                        {
                                ...
                                if (u == src)
                                {
                                        ...
                                        //minPath = getMin(vertices[v].incoming, report);
                                }
                                if(minPath == -1)
                                {
                                        report[v].npaths = 1;
                                        ...
                                        {
                                                if(report[e.vertex_id].dist == minPath)
                                                {
...
bool extract_path(const vector<vertex_label> & rpt,
        ...
        // your code here!
        for (var = dest; rpt[var].dist != 0 ; var = rpt[var].pred)
        {
                path.insert(path.begin(), var);
                if(var == -1)
                {
                        path.clear();
                        return false;
 ...
 *  runtime:  O(V+E)
 */
void setDistance(vector<vertex_label> & rpt)
{
        for (int i; i < rpt.size(); i++)
        {
                rpt[i].dist = 0;
        }
}
...
bool dag_critical_paths(vector<vertex_label> & rpt) {
        ...
        if(order.empty())
        {
                ...
                for(int i = 0; i< order.size(); ++i)
                {
                        ...
                        {
                                rpt[order[i]].pred = order[i];
                                rpt[order[i]].dist = 0;
                        }
                        for(edge &e : vertices[order[i]].outgoing)
                        {
                                cur = e.vertex_id;
                                if(rpt[cur].dist < rpt[order[i]].dist + e.weight)
                                {
                                        rpt[cur].dist = rpt[order[i]].dist + e.weight;
                                        rpt[cur].pred = rpt[order[i]].pred;
                                }
                        }
...
bool valid_topo_order(const vector<int> & order)
{
        ...
        if (vertices[order[0]].incoming.size() != 0)
        {
                return false;
        }
        else{
        ...
        {
                for(edge &e : vertices[order[counter]].incoming)
                {
                        if(visited[e.vertex_id] != true)
                        {
                                return false;
                        }
                }
                visited[order[counter]] = true;
                counter++;
        }
        return true;
}
