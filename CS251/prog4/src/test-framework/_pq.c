...
int peek_test(PQ *q, _PQ *_q) {
  ...
  if(pq_peek_top(q, &id, &p) != _pq_peek_top(_q, &_id, &_p)) {
    printf("  peek_test ERROR TYPE 1 - one queue empty, other nonempty?\n");
    return 0;
  }
  bad = 0;
  if(id != _id){ 
    bad = 1;
    printf("  peek_test ERROR TYPE 2: mismatched IDs\n");
  }
  if(p != _p){ 
    bad = 1;
    printf("  peek_test ERROR TYPE 3: mismatched priorities\n");
  }
  if(bad) {
    passed = 0;
    printf("        Expected <id=%i; p=%f>; received <id=%i, p=%f>\n", 
...
//
// expects identical results.  reports success/failure
int one_change_priority(PQ *q, _PQ *_q, int id, int newp) {
  int success, _success;
